...
func (s *AuthService) Register(ctx context.Context, email, pw, name, ip, ua string) (*vtypes.User, string, string, time.Time, error) {
	...
	if err != nil {
		return nil, "", "", time.Time{}, apperrors.Wrap(apperrors.KindInternal, err, "hash")
	}
	...
	if err != nil {
		return nil, "", "", time.Time{}, apperrors.Wrap(apperrors.KindInternal, err, "mint")
	}
...
func (s *AuthService) Refresh(ctx context.Context, token string) (string, string, time.Time, error) {
	if token == "" {
		return "", "", time.Time{}, apperrors.New(apperrors.KindUnauthorized, "refresh token required")
	}
	h := hashRefresh(token)
	...
	if rt.RevokedAt != nil {
		return "", "", time.Time{}, apperrors.New(apperrors.KindUnauthorized, "refresh token revoked")
	}
	if time.Now().After(rt.ExpiresAt) {
		return "", "", time.Time{}, apperrors.New(apperrors.KindUnauthorized, "refresh expired")
	}
...
func (s *AuthService) Logout(ctx context.Context, token, userID, ip, ua string) error {
	if token == "" {
		return apperrors.New(apperrors.KindValidation, "refresh_token required")
	}
	rt, err := s.refreshTokens.FindByHash(ctx, hashRefresh(token))
	if err != nil {
		if apperrors.Is(err, apperrors.KindUnauthorized) {
			return nil
		}
		return err
	}
	if rt.UserID != userID {
		return apperrors.New(apperrors.KindForbidden, "token does not belong to user")
	}
...
func NewProjectService(r repository.ProjectRepository, a audit.Recorder) *ProjectService {
	return &ProjectService{projects: r, audit: a}
}

func (s *ProjectService) Create(ctx context.Context, actor audit.Actor, name, desc string) (*vtypes.Project, error) {
	if name == "" {
		return nil, apperrors.New(apperrors.KindValidation, "name required")
	}
...
func (s *ProjectService) Get(ctx context.Context, uid, role, id string) (*vtypes.Project, error) {
	...
	if role != string(vtypes.RoleAdmin) && p.OwnerID != uid {
...
func (s *ProjectService) Update(ctx context.Context, actor audit.Actor, id string, name, desc *string) (*vtypes.Project, error) {
	...
	if p.OwnerID != actor.UserID {
		return nil, apperrors.ErrForbidden
	}
	newName, newDesc := p.Name, p.Description
	if name != nil {
		if *name == "" {
			return nil, apperrors.New(apperrors.KindValidation, "name cannot be empty")
		}
		newName = *name
	}
	if desc != nil {
...
func (s *AssetService) PresignUpload(ctx context.Context, actor audit.Actor, pid, filename, ctype string, size int64) (*vtypes.Asset, string, error) {
	...
	if !isImage {
		return nil, "", apperrors.New(apperrors.KindValidation, "unsupported file type")
	}
	...
	if err != nil {
		return nil, "", apperrors.Wrap(apperrors.KindStorage, err, "presign put")
	}
...
func (s *AssetService) ConfirmUpload(ctx context.Context, actor audit.Actor, id string) (*vtypes.Asset, error) {
	...
	if err != nil {
		return nil, apperrors.Wrap(apperrors.KindStorage, err, "check upload")
	}
	if !ok {
		return nil, apperrors.New(apperrors.KindValidation, "upload not found")
	}
...
func (s *AssetService) UploadDirect(ctx context.Context, actor audit.Actor, pid string, fh *multipart.FileHeader) (*vtypes.Asset, error) {
	...
	if !allowedImageExt[ext] {
		return nil, apperrors.New(apperrors.KindValidation, "unsupported extension")
	}
	if fh.Size <= 0 || fh.Size > s.limits.MaxImageUploadBytes {
		return nil, apperrors.New(apperrors.KindValidation, "size out of range")
	}
	f, err := fh.Open()
	if err != nil {
		return nil, err
	...
	if !strings.HasPrefix(detected, "image/") {
		return nil, apperrors.New(apperrors.KindValidation, "content not an image")
	}
	key := storageKey(actor.UserID, pid, ext)
	if err := s.store.Put(ctx, key, detected, f, fh.Size); err != nil {
		return nil, apperrors.Wrap(apperrors.KindStorage, err, "put")
	}
	if _, err := f.Seek(0, io.SeekStart); err != nil {
		return nil, err
	}
...
func (s *AssetService) Get(ctx context.Context, uid, role, id string) (*vtypes.Asset, error) {
	...
	if role != string(vtypes.RoleAdmin) && a.OwnerID != uid {
		return nil, apperrors.ErrForbidden
	}
	return a, nil
}
...
func (s *AssetService) DownloadURL(ctx context.Context, uid, role, id string, exp time.Duration) (string, *vtypes.Asset, error) {
	...
	if err != nil {
		return "", nil, apperrors.Wrap(apperrors.KindStorage, err, "presign get")
	}
	return u, a, nil
}
...
func validateFilename(name string) error {
	if name == "" || len(name) > 255 {
		return apperrors.New(apperrors.KindValidation, "invalid filename")
	}
	if strings.ContainsAny(name, "/\\\x00") {
		return apperrors.New(apperrors.KindValidation, "invalid filename chars")
	}
	return nil
}
...
func (s *ModelService) GetModel(ctx context.Context, id string) (*vtypes.Model, error) {
...
func (s *ModelService) CreateVersion(ctx context.Context, actor audit.Actor, mid, ver, art string, rt vtypes.Runtime, st vtypes.ModelVersionStatus, meta vtypes.JSONB) (*vtypes.ModelVersion, error) {
	if _, err := s.models.FindByID(ctx, mid); err != nil {
		return nil, err
	}
...
func (s *ModelService) SetVersionStatus(ctx context.Context, actor audit.Actor, id string, st vtypes.ModelVersionStatus) error {
	if err := s.versions.SetStatus(ctx, id, st); err != nil {
		return err
	}
...
func (s *InferenceService) CreateSync(ctx context.Context, actor audit.Actor, pid, aid, mvid string) (*vtypes.InferenceJob, *vtypes.JobResult, error) {
	if err := s.authorize(ctx, actor.UserID, pid, aid, mvid); err != nil {
		return nil, nil, err
	}
	now := time.Now().UTC()
	...
	if err != nil {
		_ = s.jobs.MarkFailed(ctx, j.ID, vtypes.ErrCodeStorage, err.Error(), nil)
		return nil, nil, err
	}
	r, _, err := s.store.Get(ctx, a.StorageKey)
	if err != nil {
		_ = s.jobs.MarkFailed(ctx, j.ID, vtypes.ErrCodeStorage, err.Error(), nil)
		return nil, nil, apperrors.Wrap(apperrors.KindStorage, err, "fetch asset")
	}
	defer r.Close()
	buf := &bytes.Buffer{}
	io.Copy(buf, r)
	...
	if err != nil {
		_ = s.jobs.MarkFailed(ctx, j.ID, vtypes.ErrCodeMLService, err.Error(), nil)
		return nil, nil, apperrors.Wrap(apperrors.KindInference, err, "ml")
	}
	inferMs := resp.InferenceTimeMs
	if inferMs == 0 {
		inferMs = time.Since(start).Milliseconds()
	}
	...
	if _, err := s.results.Create(ctx, &vtypes.InferenceResult{JobID: j.ID, ModelVersionID: mvid, ResultJSON: raw, ProcessingTimeMs: total, InferenceTimeMs: inferMs}); err != nil {
		_ = s.jobs.MarkFailed(ctx, j.ID, vtypes.ErrCodeDatabase, err.Error(), nil)
		return nil, nil, err
	}
	comp := time.Now().UTC()
	if err := s.jobs.MarkSucceeded(ctx, j.ID, comp); err != nil {
		slog.Warn("mark success failed", "error", err)
	}
...
func (s *InferenceService) CreateAsync(ctx context.Context, actor audit.Actor, pid, aid, mvid string, prio vtypes.Priority, idem *string) (*vtypes.InferenceJob, error) {
	...
	if idem != nil && *idem != "" {
		if ex, err := s.jobs.FindByIdempotencyKey(ctx, pid, *idem); err == nil {
			return ex, nil
		}
	}
	...
	if err := s.jobs.MarkQueued(ctx, j.ID, 1, queuedAt); err != nil {
		return nil, err
	}
...
func (s *InferenceService) Get(ctx context.Context, uid, role, jid string) (*vtypes.InferenceJob, error) {
	...
	if err := s.authzProject(ctx, uid, role, j.ProjectID); err != nil {
		return nil, err
	}
	return j, nil
}
func (s *InferenceService) GetResult(ctx context.Context, uid, role, jid string) (*vtypes.JobResult, error) {
	...
	if err := json.Unmarshal(r.ResultJSON, &jr); err != nil {
		return nil, apperrors.Wrap(apperrors.KindInternal, err, "decode result")
	}
	return &jr, nil
}
func (s *InferenceService) List(ctx context.Context, uid, role, pid string, statuses []vtypes.JobStatus, cursor string, limit int) ([]vtypes.InferenceJob, string, bool, error) {
	if err := s.authzProject(ctx, uid, role, pid); err != nil {
		return nil, "", false, err
	}
...
func (s *InferenceService) Retry(ctx context.Context, actor audit.Actor, jid string) (*vtypes.InferenceJob, error) {
	...
	if err := s.authzProject(ctx, actor.UserID, "", j.ProjectID); err != nil {
		return nil, err
	}
	if j.Status != vtypes.JobStatusFailed && j.Status != vtypes.JobStatusDead {
		return nil, apperrors.New(apperrors.KindConflict, "only FAILED/DEAD can be retried")
	}
	if err := s.jobs.MarkRetrying(ctx, j.ID); err != nil {
		return nil, err
	}
	attempt := j.Attempts + 1
	if err := s.jobs.MarkQueued(ctx, j.ID, attempt, time.Now().UTC()); err != nil {
		return nil, err
	}
...
func (s *InferenceService) Stats(ctx context.Context, uid string) (*DashboardStats, error) {
	...
	mlOK := true
	if _, err := s.ml.Health(ctx); err != nil {
		mlOK = false
	}
	storeOK := true
...
func (s *InferenceService) authorize(ctx context.Context, uid, pid, aid, mvid string) error {
	if err := s.authzProject(ctx, uid, "", pid); err != nil {
		return err
	}
	a, err := s.assets.FindByID(ctx, aid)
	if err != nil {
	...
	if a.ProjectID != pid {
		return apperrors.New(apperrors.KindValidation, "asset not in project")
	}
