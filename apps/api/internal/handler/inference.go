...
func (h *Handler) AsyncInference(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil { fail(c, errBind(err)); return }
	var idem *string
	if k := strings.TrimSpace(c.GetHeader("Idempotency-Key")); k != "" { idem = &k }
	prio := req.Priority
	if prio == 0 { prio = vtypes.PriorityNormal }
...
func (h *Handler) ListProjectJobs(c *gin.Context) {
	...
	for _, s := range c.QueryArray("status") { statuses = append(statuses, vtypes.JobStatus(s)) }
