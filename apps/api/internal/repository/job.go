...
func (r *jobRepo) Create(ctx context.Context, j *vtypes.InferenceJob) (*vtypes.InferenceJob, error) {
	if j.CreatedAt.IsZero() { j.CreatedAt = now() }
	if j.UpdatedAt.IsZero() { j.UpdatedAt = j.CreatedAt }
	if j.Status == "" { j.Status = vtypes.JobStatusCreated }
	if j.MaxAttempts <= 0 { j.MaxAttempts = 5 }
	var idem interface{}
	if j.IdempotencyKey != nil && *j.IdempotencyKey != "" { idem = *j.IdempotencyKey }
	...
	if err != nil {
		if uniqueViolation(err) { return nil, apperrors.New(apperrors.KindConflict, "duplicate idempotency key") }
...
func (r *jobRepo) List(ctx context.Context, projectID string, statuses []vtypes.JobStatus, cursor string, limit int) ([]vtypes.InferenceJob, string, bool, error) {
	...
	if len(statuses) > 0 {
		...
		for i, s := range statuses { ss[i] = string(s) }
		fmt.Fprintf(&qb, " AND status = ANY($%d)", argn)
		args = append(args, pq.Array(ss)); argn++
	}
	if cursor != "" {
	...
	if err != nil { return nil, "", false, failExec(apperrors.KindDatabase, err, "list jobs") }
	defer rows.Close()
...
func (r *jobRepo) transition(ctx context.Context, id string, current, next vtypes.JobStatus, extra string, args ...interface{}) (bool, error) {
	...
	if err != nil { return false, failExec(apperrors.KindDatabase, err, "transition to "+string(next)) }
...
func (r *jobRepo) MarkQueued(ctx context.Context, id string, attempts int, queuedAt time.Time) error {
	...
	if err != nil || !ok2 { return apperrors.New(apperrors.KindConflict, "job not in queuable state") }
	return nil
}
...
func (r *jobRepo) MarkSucceeded(ctx context.Context, id string, completedAt time.Time) error {
	...
	if err != nil || !ok { return apperrors.New(apperrors.KindConflict, "job not in RUNNING state") }
	return nil
}
func (r *jobRepo) MarkFailed(ctx context.Context, id string, code vtypes.ErrorCode, message string, completedAt *time.Time) error {
	...
	if completedAt != nil { extra += `, completed_at=$6`; args = append(args, *completedAt) }
...
func (r *jobRepo) MarkRetrying(ctx context.Context, id string) error {
	...
	if err2 != nil || !ok2 { return apperrors.New(apperrors.KindConflict, "job not retryable") }
	return nil
}
func (r *jobRepo) MarkDead(ctx context.Context, id string, code vtypes.ErrorCode, message string) error {
	...
	if err != nil || !ok { return apperrors.New(apperrors.KindConflict, "job not in RETRYING state") }
	return nil
}
func (r *jobRepo) IncrementAttempts(ctx context.Context, id string) (int, error) {
	...
	if err != nil { return 0, failExec(apperrors.KindDatabase, err, "increment attempts") }
	return a, nil
}
func (r *jobRepo) PickNextQueued(ctx context.Context) (*vtypes.InferenceJob, error) {
	...
	if err != nil {
		if apperrors.Is(err, apperrors.KindNotFound) { return nil, nil }
		return nil, err
	}
	return j, nil
}
func (r *jobRepo) CountByStatus(ctx context.Context) (map[vtypes.JobStatus]int64, error) {
	...
	if err != nil { return nil, failExec(apperrors.KindDatabase, err, "count by status") }
	defer rows.Close()
	out := map[vtypes.JobStatus]int64{}
	for rows.Next() {
		var s string; var n int64
		if err := rows.Scan(&s, &n); err != nil { return nil, err }
...
func (r *jobRepo) CountQueueDepth(ctx context.Context) (int64, error) {
	...
	if err != nil { return 0, failExec(apperrors.KindDatabase, err, "queue depth") }
	return n, nil
}
...
func scanJob(row rowScanner) (*vtypes.InferenceJob, error) {
	...
	if err != nil {
		if stderrors.Is(err, sql.ErrNoRows) { return nil, notFound("job") }
		return nil, failExec(apperrors.KindDatabase, err, "scan job")
	}
	j.Status = vtypes.JobStatus(status); j.Priority = vtypes.Priority(prio)
	if idem.Valid { s := idem.String; j.IdempotencyKey = &s }
	if errCode.Valid { c := vtypes.ErrorCode(errCode.String); j.ErrorCode = &c }
	if errMsg.Valid { s := errMsg.String; j.ErrorMessage = &s }
	if queuedAt.Valid { j.QueuedAt = &queuedAt.Time }
	if startedAt.Valid { j.StartedAt = &startedAt.Time }
	if comp.Valid { j.CompletedAt = &comp.Time }
	return j, nil
}
